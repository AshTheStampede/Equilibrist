...
def parse_script(text: str) -> dict:
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("$"):
            ...
            elif section_raw.startswith("temperature"):
                section = "temperature"
                # Support inline syntax: $temperature 50 C  or  $temperature 300 K
                _inline = section_raw[len("temperature"):].strip()
                if _inline:
                    _iparts = _inline.split()
                    try:
                        _ival  = float(_iparts[0])
                        _iunit = _iparts[1] if len(_iparts) > 1 else "K"
                        result["temperature_K"] = convert_temperature_to_K(_ival, _iunit)
                    except (ValueError, IndexError):
                        result["_temperature_warning"] = (
                            f"Could not parse inline temperature '${section_raw}'. "
                            f"Use: $temperature 25 C  or  $temperature 300 K")
        ...
        if section == "temperature":
            # Accepts: "T = 300 K", "T = 25 C", or bare "300 K" / "25 C"
            ...
            if m:
                ...
                try:
                    ...
                    result["_temperature_warning"] = (
                        f"Could not parse temperature '{line}'. "
                        f"Use: T = 25 C  or  T = 300 K")
