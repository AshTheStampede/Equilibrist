    ...
    try:
        ...
        if not fitting_concs:
            for v, _, _ in exp_points:
                x_val = convert_exp_x(np.array([v]), x_expr, parsed, params, network)[0]
                        ...
                        try:
                            if fitting_concs:
                                x_val = convert_exp_x(np.array([v_add]), x_expr,
                                                      cur_parsed, cur_params, network)[0]
        ...
        try:
            ...
            for v_add, exp_y, exp_col in exp_points:
                try:
                    x_val = convert_exp_x(np.array([v_add]), x_expr,
                                          fin_parsed, fin_params, network)[0]
            ...
            def raw_rvec(fp):
                ...
                for v_add, exp_y, exp_col in exp_points:
                    try:
                        xv = convert_exp_x(np.array([v_add]), x_expr, cpd, cp, network)[0]
